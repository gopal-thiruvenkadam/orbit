...
const statusConfig = {
  ...
  todo: { color: 'action', icon: <WarningIcon color="action" /> },
  blocked: { color: 'error', icon: <WarningIcon color="error" /> }
} as const;

const ImplementationPhase: React.FC = () => {
  ...
  const currentPhase = workflowPhases.find((p: any) => p.phaseType === 'implementation' || p.phaseType === 'IMPLEMENTATION');
  const tasks = currentPhase ? currentPhase.tasks.map((t: any) => ({
    id: t.id,
    name: t.title,
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" paragraph>
        This phase focuses on transforming design specifications into functioning code, establishing development environments, and implementing core functionality.
      </Typography>
          ...
          {tasks.length > 0 ? tasks.map((task: any) => (
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 2, textAlign: 'center' }}>
                <Typography color="text.secondary">No tasks found for this phase in this project.</Typography>
              </Paper>
            </Grid>
          )}
        </Grid>
